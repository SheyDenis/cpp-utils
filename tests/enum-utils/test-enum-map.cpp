...
struct DummyEnumEnumMap {
  ...
  static std::array constexpr values = {
      std::pair<key_type, value_type>{key_type::FIRST, "FIRST"},
      std::pair<key_type, value_type>{key_type::SECOND, "SECOND"},
      std::pair<key_type, value_type>{key_type::THIRD, "THIRD"},
      std::pair<key_type, value_type>{key_type::FOURTH, "FOURTH"},
...
struct DummyEnumEnumMapStringView {
  using key_type = DummyEnum;
...
template <typename T>
typename T::value_type dummy_enum_to_value_type(DummyEnum const val) {
...
TEST_CASE("Test with DummyEnumEnumMap") {
  ...
  for (auto const& [enum_val, str_val] : test_data) {
    value_type const result = dummy_enum_to_value_type<enum_type>(enum_val);
    REQUIRE_THAT(result, Catch::Matchers::Equals(str_val));
  }
}

TEST_CASE("Test with DummyEnumEnumMapStringView") {
  using enum_type = DummyEnumEnumMapStringView;
  using value_type = enum_type::value_type;
  ...
  for (auto const& [enum_val, str_val] : test_data) {
    value_type const result = dummy_enum_to_value_type<enum_type>(enum_val);
    REQUIRE_THAT(result.data(), Catch::Matchers::Equals(str_val));
  }
}
